      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="card p-8 border-white/10 bg-surface/50 backdrop-blur-xl">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400 mt-1">{t('auth.loginSub')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button type="submit" disabled={isLoading} className="btn-primary w-full py-3.5 text-lg shadow-lg shadow-primary/25">
              {isLoading ? <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : t('auth.loginBtn')}
            </button>
          </form>

          <div className="mt-6 text-center text-sm text-gray-400">
            {t('auth.noAccount')} <Link to="/register" className="text-primary hover:text-primary-400 font-medium transition-colors">{t('auth.registerLink')}</Link>
          </div>
        </div>
      </motion.div>
    </div>
