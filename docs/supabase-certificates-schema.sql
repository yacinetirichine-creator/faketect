...
-- Commentaires
COMMENT ON TABLE certificates IS 'Certificats premium avec QR code et explications détaillées';
COMMENT ON COLUMN certificates.verdict IS 'Verdict en majuscules : AUTHENTIQUE, SUSPECT, INCERTAIN, FAUX';
COMMENT ON COLUMN certificates.score IS 'Pourcentage de probabilité IA (0-100)';
COMMENT ON COLUMN certificates.file_path IS 'Chemin du fichier PDF sur le serveur';
COMMENT ON COLUMN certificates.verify_url IS 'URL de vérification publique (avec QR code)';
